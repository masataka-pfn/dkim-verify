...
my $mfile = "";
#$mfile = '/mnt/c/temp/iij-email.txt';
#$mfile = '/mnt/c/temp/sample3.txt';
#$mfile = '/mnt/c/temp/smbc1.txt';
$mfile = '/mnt/c/temp/nikkei01.txt';		# Verify Success

#	Woring files
my $hash_target_f = "hash_target.txt";
my $body_hash_f = "mail_body_canonical.txt";
my $mail_body_f = "mail_body.txt";
my $pub_key_f = "pub_key.pem";
my $sig_f = "sig.sig";
...
#die "NO dkim definition\n" if(scalar(@$dkim_rec) <= 0);
&gen_file($mail_body, $mail_body_f);
...
&remove_last_blank_lines($canonical);
my $canonical_body = join("\r\n", @$canonical) . "\r\n";
#$canonical_body =~ s/[\r\n]+$//;		# remove DKIM-signature
&gen_file($canonical_body, $body_hash_f);

my $bh = `cat $body_hash_f | openssl dgst -sha256 -binary | base64`;
chop($bh);
